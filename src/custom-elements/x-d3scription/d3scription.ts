...
function getOffset(event: MouseEvent, bounds: ClientRect, offset: Position): Position {
    const collideVertically: boolean =
        windowDimensions.height +
            ...
            bounds.height <
        0;
    const collideHorizontally: boolean =
    ...
    return {
        top: collideVertically
            ? event.pageY - bounds.height - offset.top
            : offset.top + event.pageY,
        left: collideHorizontally
            ? event.pageX - bounds.width - offset.left
...
export default function d3scription<T>(
    contentGetter: ContentGetter<T>,
    options: Options = {}
): TipFactory<T> {
    const offsetSettings: Position = getOffsetSettings(options.offset);

    return function (): Tip<T> {
        const tip: d3.Selection<any> = d3
            .select("body")
            .append("div")
        ...
        function updateTipPosition() {
            ...
            tip.style("top", `${position.top}px`).style("left", `${position.left}px`);
        }
